...
const StorageLocker = () => {
  ...
  const debouncedLoadMore = debounce((nativeEvent) => {
  ...
  }, 300); // Delay 300ms cho debounce
  
  const loadMore = (event) => {
    const { nativeEvent } = event;
    debouncedLoadMore(nativeEvent); // Truyền nativeEvent thay vì toàn bộ sự kiện
  };
      ...
      {storageLockers.map((storage) => (
        <View key={storage.id} style={styles.storageContainer}>
          ...
          />
            ...
            <ScrollView onScroll={loadMore} refreshControl={<RefreshControl
              ...
              {packages.map((packages) => (
                <Card key={packages.id} style={styles.card}>
                  <Card.Content>
                    ...
                    {packages.pickup_time && (
                      <Text style={styles.dateText}>
                        Thời gian nhận hàng:{" - "} 
                        {new Date(packages.pickup_time).toLocaleDateString("vi-VN", {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
