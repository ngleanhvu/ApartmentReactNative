...
import { Card, Title, Paragraph, Badge, Text, ActivityIndicator, Searchbar } from 'react-native-paper';
        ...
        <View style={styles.container}>
            {storageLockers.map((storage) => (
                <View key={storage.id} style={styles.storageContainer}>
                    ...
                    <Searchbar placeholder="Tìm kiếm đơn hàng..." value={q} onChangeText={k => search(k, setQ)} style={styles.searchBar}/>
                        ...
                        <Card.Content>
                            <View style={styles.packageHeader}>
                                <View style={styles.packageInfo}>
                                    ...
                                    <Paragraph>Người nhận: {packages.recipient_name}</Paragraph>
                                    <Paragraph>Số lượng sản phẩm: {packages.quantity_items}</Paragraph>
                                </View>

                                <Badge
                                    style={[
                                        styles.status,
                                        { backgroundColor: packages.status === 'Not received' ? 'red' : 'green' }
                                    ]}
                                >
                                    {packages.status}
                                </Badge>
                            ...
                            {packages.description && (
                                <Text style={styles.description}>*Ghi chú: {packages.description.replace(/<[^>]*>/g, '')}
                                </Text>
                            )}
                        </Card.Content>
                    </Card>
