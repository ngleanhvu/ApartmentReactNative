...
import { Card, Text, RadioButton, Modal, Button, Badge, Avatar, IconButton} from "react-native-paper";
                    ...
                    <View key={s.id}>
                        <Card style={styles.card} onPress={() => handleCardPress(s)}>
                            <Card.Title
                                ...
                                left={(props) => <Avatar.Icon {...props} icon="database-search-outline" />}
                            />
                            <Card.Content>
                                <View>
                                    <Badge 
                                        style={[
                                            styles.status,
                                            { backgroundColor: s.status === 'Published' ? 'green' : 'red' }
                                        ]}
                                    >
                                        Tình trạng: {s.status}
                                    </Badge>
                                </View>
                                <Text style={styles.description}>Nội dung: {s.description}</Text>
                                <Text style={styles.dateText}>
                                    Thời gian : {new Date(s.start_date).toLocaleDateString("vi-VN")} - {new Date(s.end_date).toLocaleDateString("vi-VN")}
                                </Text>
                            </Card.Content>
                        </Card>
                    </View>
            ...
            >
                <View style={styles.modalContainer}>
                    {selectedSurvey && selectedSurvey.questions && (
                        <>
                            <Text style={styles.modalTitle}>Khảo sát về:  {selectedSurvey.title} </Text>
